...
This project implements inventory models from *Logic of Logistics*, as a way for me to implement operations research and optimization to inventory management.
...
Reproduce the environment with `uv sync` and run the suite with `uv run pytest`. Current models assume deterministic demand, instantaneous replenishment, and no shortages; extending the network model to time-varying purchase costs, lead times, and service constraints is future work.
